...
type inMemExtCache struct {
	items         xsync.MapOf[string, *extCacheItem]
...
func NewInMemExtendedCache() razcache.ExtendedCache {
	cache := &inMemExtCache{
		items:         *xsync.NewMapOf[string, *extCacheItem](),
...
func (c *inMemExtCache) Del(key string) error {
	c.items.Delete(key)
	return nil
}

func (c *inMemExtCache) GetTTL(key string) (time.Duration, error) {
	item, ok := c.items.Load(key)
	if !ok {
...
func (c *inMemExtCache) SetTTL(key string, ttl time.Duration) error {
	item, ok := c.items.Load(key)
	if !ok {
		return razcache.ErrNotFound
	}
...
func (c *inMemExtCache) Close() error {
	c.closed.Store(true)
